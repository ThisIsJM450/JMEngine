...
void FrameResources::BindCommon(ID3D11DeviceContext* context)
{
    ...
    context->GSSetSamplers(0, 3, samplers);
}
...
void FrameResources::UpdateBones(ID3D11DeviceContext* ctx, const DirectX::XMFLOAT4X4* bones, uint32_t boneCount)
{
    ...
    // HLSL matrix 기본(column-major)과 맞추기 위해 업로드 직전에 transpose
    std::vector<DirectX::XMFLOAT4X4> bonesForGPU;
    bonesForGPU.resize(boneCount);
    for (uint32_t i = 0; i < boneCount; ++i)
    {
        DirectX::XMStoreFloat4x4(
            &bonesForGPU[i],
            DirectX::XMMatrixTranspose(DirectX::XMLoadFloat4x4(&bones[i])));
    }

    // 1) StructuredBuffer 업데이트
    UpdateSB(ctx, m_BoneSB.Get(), bonesForGPU.data(), sizeof(DirectX::XMFLOAT4X4) * boneCount);
