...
void GPUMeshSkeletal::Init(ID3D11Device* dev, const CpuMeshBase* cpu)
{
    if (!dev || !cpu)
        return;
    ...
    vbDesc.CPUAccessFlags = 0; // 0 if no CPU access is necessary.
...
void GPUMeshSkeletal::Refresh(SceneProxyBase* procxy)
{
    GPUMeshBase::Refresh(procxy);
    if (SkeletalMeshSceneProxy* SKProxy = static_cast<SkeletalMeshSceneProxy*>(procxy))
    {
        if (SKProxy->bBoneDirty)
        {
            BonePalette = SKProxy->BonePalette;
            SKProxy->bBoneDirty = false;
        }
    }
}
