    ...
    SkeletalMeshAsset& sk)
{
    ...
    for (size_t i = 0; i < sk.BonePalette.size(); ++i)
    {
        XMStoreFloat4x4(&sk.BonePalette[i], XMMatrixIdentity());
    }
...
bool FbxImporter::ImportFBX(const std::string& path, const ImportOptions& opt, ImportResult& out)
{
    ...
    if (opt.bBuildMaterials)
    {
        device = Dx11Context::Get().GetDevice();
        if (!device)
        {
            return false;
        }
