...
static bool IsNearlyUniformScale_Skeletal(const DirectX::XMFLOAT3& s, float eps = 1e-4f)
{
...
{
    m_Mesh = mesh;
    m_bConsumeRootInPoseForcedOffByScale = false;
    m_bPrevConsumeRootInPoseByScale = true;
    m_FootIKState = {};
    ...
    if (m_Mesh)
    {
        ...
        // 런타임 row-convention 기준으로 초기 팔레트 구성
...
void SkeletalMeshComponent::Tick(float dt)
{
    ...
    // 비균일 스케일에서는 RootMotion+RootLock을 금지한다.
    bool bAllowRootMotionByScale = IsNearlyUniformScale_Skeletal(GetRelativeTransform().m_Scale);
    if (Actor* owner = GetOwner())
    {
        if (SceneComponent* root = owner->GetRootComponent())
        {
            bAllowRootMotionByScale = bAllowRootMotionByScale &&
                IsNearlyUniformScale_Skeletal(root->GetRelativeTransform().m_Scale);
        }
    }

    if (!bAllowRootMotionByScale)
    {
        if (!m_bConsumeRootInPoseForcedOffByScale)
        {
            m_bPrevConsumeRootInPoseByScale = m_Anim->GetConsumeRootInPose();
            m_bConsumeRootInPoseForcedOffByScale = true;
        }
        m_Anim->SetConsumeRootInPose(false);
    }
    else if (m_bConsumeRootInPoseForcedOffByScale)
    {
        m_Anim->SetConsumeRootInPose(m_bPrevConsumeRootInPoseByScale);
        m_bConsumeRootInPoseForcedOffByScale = false;
    }

    m_Anim->Tick(dt);

    std::vector<DirectX::XMFLOAT4X4> correctedLocalPose = m_Anim->GetLocalPose();

    BuildGlobalPose_FromLocalPose(correctedLocalPose);
    FootIKSample::Update(*this, m_GlobalPose, dt, m_FootIKState);
    FootIKSample::ApplyPoseCorrection(*this, correctedLocalPose, dt, m_FootIKState);

    BuildFinalPalette_FromLocalPose(correctedLocalPose);
...
void SkeletalMeshComponent::BuildFinalPalette_FromLocalPose(const std::vector<DirectX::XMFLOAT4X4>& localPose)
{
    ...
    // FinalPalette (row-vector convention):
    // final = Offset * Global * GlobalInverse
    for (uint32_t i = 0; i < n; ++i)
    {
        m_FinalPalette[i] = MulM(MulM(skel.Bones[i].Offset, m_GlobalPose[i]), skel.GlobalInverse);
...
void SkeletalMeshComponent::BuildGlobalPose_FromLocalPose(const std::vector<DirectX::XMFLOAT4X4>& localPose)
{
    ...
    while (!q.empty())
    {
        ...
        {
            // row-vector convention: childGlobal = childLocal * parentGlobal
            nodeGlobal[c] = MulM(nodeLocal[c], nodeGlobal[cur]);
            visited[c] = 1;
            q.push(c);
        }
    }
