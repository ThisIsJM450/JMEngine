...
{
public:
    void SetViewInfo(const ViewInfo& viewInfo) { m_ViewInfo = viewInfo; }
    ...
    void AddPointLight(PointLightComponent* c)
    {
        if (!c) return;
        if (std::find(m_PointLights.begin(), m_PointLights.end(), c) == m_PointLights.end())
            m_PointLights.push_back(c);
    }

    void RemovePointLight(PointLightComponent* c)
    {
        auto it = std::remove(m_PointLights.begin(), m_PointLights.end(), c);
